...
{category:"Exchange / Mailbox",icon:"\u{1F4E7}",color:"#ef4444",items:[
  {id:"shared-mailboxes",name:"Shared Mailboxes",desc:"All shared mailboxes",ex:true,command:`Get-EXOMailbox -RecipientTypeDetails SharedMailbox -ResultSize Unlimited|Select-Object DisplayName,PrimarySmtpAddress,Alias,WhenCreated`,tags:["shared"]},
  {id:"mail-forwarding",name:"Mailbox Forwarding",desc:"Server-side forwarding configured",ex:true,command:`Get-EXOMailbox -ResultSize Unlimited -Property DisplayName,PrimarySmtpAddress,ForwardingAddress,ForwardingSmtpAddress,DeliverToMailboxAndForward|Where-Object{$_.ForwardingAddress -or $_.ForwardingSmtpAddress}|Select-Object DisplayName,PrimarySmtpAddress,ForwardingAddress,ForwardingSmtpAddress,DeliverToMailboxAndForward`,tags:["forwarding"]},
  ...
  {id:"mailbox-sizes",name:"Mailbox Sizes (Top 50)",desc:"Largest mailboxes",ex:true,command:`Get-EXOMailbox -ResultSize Unlimited|Get-EXOMailboxStatistics|Select-Object DisplayName,@{N='TotalSize';E={$_.TotalItemSize.ToString()}},ItemCount|Sort-Object{if($_.TotalItemSize.Value){$_.TotalItemSize.Value.ToBytes()}else{0}} -Descending|Select-Object -First 50`,tags:["size"]},
...
$mbx=Get-EXOMailbox -Identity $u -Properties LitigationHoldEnabled,ArchiveStatus,ForwardingSmtpAddress,ForwardingAddress,ProhibitSendQuota,IssueWarningQuota,WhenCreated
$st=Get-EXOMailboxStatistics -Identity $u
[PSCustomObject]@{DisplayName=$mbx.DisplayName;PrimarySmtp=$mbx.PrimarySmtpAddress;Type=$mbx.RecipientTypeDetails;TotalSize=$(if($st.TotalItemSize){$st.TotalItemSize.ToString()}else{'0'});Items=$st.ItemCount;ArchiveStatus=$mbx.ArchiveStatus;LitigationHold=$mbx.LitigationHoldEnabled;ForwardingSmtp=$mbx.ForwardingSmtpAddress;ForwardingAddress=$mbx.ForwardingAddress;WarningQuota=$mbx.IssueWarningQuota.ToString();SendQuota=$mbx.ProhibitSendQuota.ToString();Created=$mbx.WhenCreated;LastLogon=$st.LastLogonTime}`,tags:["mailbox","user","size","quota"],params:[{key:"UPN",label:"Mailbox",picker:"users"}]},
